...
<div class="sk-wrap">
  <div class="sk-container">
    ...
    <nav class="sk-crumbs" aria-label="Breadcrumb">
      ...
      <span class="sep">/</span>
      <span>New Syarat & Ketentuan</span>
    </nav>

    <!-- Head -->
    <header class="sk-head">
      <h1>New Syarat & Ketentuan</h1>
      <p>Tambahkan S&K yang akan tampil di aplikasi <strong>I Love Emas</strong>.</p>
    </header>

    <!-- Card -->
    <section class="sk-card" aria-label="Form New S&K">
      <div class="sk-card-hd">
        <h2>Form S&K</h2>
      </div>
      <div class="sk-card-bd">
        <form class="sk-form" action="<?= base_url('master/save-memo') ?>" method="post" id="formMemo">
          <div class="form-group">
            <label for="tm_value">Isi Syarat & Ketentuan</label>
            <textarea id="tm_value" class="form-control summernote" name="dt[tm_value]"></textarea>
          </div>

          <div class="form-group">
            <label for="tm_priority">Prioritas</label>
            <input type="number" id="tm_priority" class="form-control" name="dt[tm_priority]" required min="0" step="1" placeholder="Contoh: 1">
...
(function waitForDeps(){
  ...
  // Keyboard plugin (opsional—jika tersedia)
  if ($.keyboard) {
    try { $('#tm_priority').keyboard({ layout: 'qwerty' }); } catch(e){}
    // Untuk editor rich text (summernote) tidak perlu keyboard.
  }

  // Submit UX: cegah double submit
