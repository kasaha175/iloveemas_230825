...
<head>
  ...
  <!-- Optional: Google Fonts (can be removed if you prefer local fonts) -->
  ...
  <style>
    ...
    *{ box-sizing: border-box; }
    ...
    body{
      ...
      color: var(--text);
      background: radial-gradient(1200px 800px at 10% 10%, var(--blue-pastel) 0%, #e8fbff 35%, #f6fbff 55%, #fbfdff 70%, #ffffff 100%);
      overflow: hidden;
    }
    ...
    .hero::before{ /* subtle texture / image overlay (optional) */
      content:""; position:absolute; inset:0; opacity:.18;
      background: url('<?= base_url() ?>assets/img/back.jpg') center/cover no-repeat fixed;
    }
    ...
    .brand-title{ font-size: clamp(18px, 2.2vw, 24px); font-weight:600; letter-spacing:.3px; opacity:.95; }
    ...
    .input{ width:100%; padding: 14px 44px 14px 14px; border-radius: 12px; border:1px solid #dfe7ff;
      background:#fff; outline:none; font-size:15px; transition: border .2s, box-shadow .2s; }
    .input:focus{ border-color: var(--blue-light); box-shadow: 0 0 0 4px rgba(7,71,153,.12); }

    .floating-label{ position:absolute; left:14px; top:50%; transform: translateY(-50%);
    ...
    .input:not(:placeholder-shown) + .floating-label{
      top:0; transform: translateY(-50%) scale(.88); color: var(--blue-light); background:#fff; }

    .suffix-btn{ position:absolute; right:8px; top:50%; transform: translateY(-50%);
      border:none; background:transparent; padding:6px 10px; border-radius:10px; cursor:pointer; }
    ...
    .btn{
      ...
      border-radius:12px; padding:12px 16px; font-weight:600; font-size:15px; letter-spacing:.2px;
      color:#fff; background: linear-gradient(135deg, var(--blue-light), var(--blue-dark));
      box-shadow: 0 10px 20px rgba(0,26,110,.25); transition: transform .06s ease, box-shadow .2s ease; }
    ...
    /* ====== Alert ====== */
    .alert{
      display:flex; align-items:flex-start; gap:10px; padding:10px 12px; border-radius:12px; font-size:14px;
      margin-top:14px; background: #fff4f4; border:1px solid #ffd7d7; color:#9a1a1a;
    }
    ...
    @media (max-width: 560px){ .auth-card{ background:#ffffff; } }
  </style>
</head>
<body>
  <main class="auth-page">
    ...
    <section class="hero" aria-label="Brand">
      ...
      <div class="hero-footer">© <?= date('Y') ?> — All rights reserved.</div>
    ...
    <section class="panel" aria-label="Login form">
      <div class="auth-card" role="dialog" aria-labelledby="loginTitle" aria-describedby="loginDesc">
        <header class="auth-header">
          <img src="<?= base_url() ?>assets/img/logo.png" alt="Logo" class="auth-logo" />
          <h2 id="loginTitle" class="auth-title">Masuk ke Akun</h2>
          <p id="loginDesc" class="auth-sub">Silakan masukkan kredensial Anda</p>
        </header>

        <form action="<?= base_url('login-process') ?>" method="post" novalidate>
          <!-- CSRF (aktifkan di config) -->
        ...
        <?php if ($this->session->userdata('failedLogin') === true): ?>
          <div class="alert" role="alert" aria-live="polite">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><path d="M12 9v4m0 4h.01M12 2a10 10 0 100 20 10 10 0 000-20z" stroke="#dc2626" stroke-width="2" stroke-linecap="round"/></svg>
            <span>Username atau password salah.</span>
          </div>
          <?php $this->session->set_userdata(['failedLogin' => false]); ?>
        <?php endif; ?>

        <div class="meta">Keamanan dilindungi; gunakan kredensial yang kuat.</div>

        <!-- Optional: On-screen keyboard init (works only if jQuery Keyboard plugin is included on the page) -->
        <div class="meta" style="margin-top:8px;">V3.0.0</div>
      </div> 
    </section>
  </main>
